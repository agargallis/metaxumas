export const recurringEvents = [
  {
    id: 'r1',
    day: 'Παρασκευή',
    dayShort: 'Παρ',
    title: 'Ζωντανή Μουσική Βραδιά',
    description: 'Καλλιτέχνες: Σωτήρης Παπαϊωάννου - Χρήστος Ιατρού - Πέτρος Τριανταφύλλου - Σοφία Χριστακάκου',
    time: '21:30',
    genre: 'Έντεχνα λαϊκά / Ρεμπέτικα',
    vibe: 'Ζεστό και παρεΐστικο',
    artist: null,
    reservationRequired: true,
    highlight: true,
  },
  {
    id: 'r2',
    day: 'Σάββατο',
    dayShort: 'Σαβ',
    title: 'Σαββατιάτικη Μουσική Βραδιά',
    description: 'Καλλιτέχνες: Δημήτρης Βλάχος - Δημήτρης Ζαβογιάννης - Μαριαλένα Μαρούλη - Χρήστος Ιατρού',
    time: '21:30',
    genre: 'Παλαιά λαϊκά / Ρεμπέτικα',
    vibe: 'Γιορτινό και ζωντανό',
    artist: null,
    reservationRequired: true,
    highlight: true,
  },
  {
    id: 'r3',
    day: 'Κυριακή',
    dayShort: 'Κυρ',
    title: 'Κυριακάτικη Μουσική Ημέρα',
    description: 'Καλλιτέχνες:  Σωτήρης Παπαϊωάννου - Πέτρος Τριανταφύλλου - Δημήτρης Βλάχος - Δημήτρης Ζαβογιάννης',
    time: '15:00',
    genre: 'Παλαιά λαϊκά / Ρεμπέτικα',
    vibe: 'Χαλαρό και παρεΐστικο',
    artist: null,
    reservationRequired: true,
    highlight: true,
  },
  {
    id: 'r4',
    day: 'Τετάρτη',
    dayShort: 'Τετ',
    title: 'Acoustic Βραδιά',
    description: 'Μια πιο ήπια μουσική βραδιά με ακουστικό ήχο και ατμόσφαιρα για κρασί και κουβέντα.',
    time: '20:30',
    genre: 'Acoustic / Indie Greek',
    vibe: 'Ήπιο και ατμοσφαιρικό',
    artist: null,
    reservationRequired: false,
    highlight: false,
  },
]

export const upcomingEvents = [
  {
    id: 'u1',
    date: '2026-04-10',
    title: 'Βραδιά Ρεμπέτικου',
    description: 'Μια ξεχωριστή βραδιά αφιερωμένη στο ρεμπέτικο, με παραδοσιακά όργανα και αυθεντική ατμόσφαιρα.',
    time: '21:00',
    genre: 'Ρεμπέτικο',
    artist: '[Καλλιτέχνης]',
    capacity: 60,
    price: 'Ελεύθερη είσοδος',
    reservationRequired: true,
    tags: ['Ρεμπέτικο', 'Παράδοση', 'Live'],
  },
  {
    id: 'u2',
    date: '2026-04-17',
    title: 'Ειδική Βραδιά με Ελληνικό Πρόγραμμα',
    description: 'Ελληνικές μελωδίες, μεζέδες και ζεστή ατμόσφαιρα για μια βραδιά που στήνεται γύρω από την παρέα.',
    time: '20:30',
    genre: 'Ελληνική Λαϊκή',
    artist: '[Καλλιτέχνης]',
    capacity: 80,
    price: 'Ελεύθερη είσοδος',
    reservationRequired: true,
    tags: ['Ελληνικό πρόγραμμα', 'Παρέα', 'Live'],
  },
  {
    id: 'u3',
    date: '2026-04-24',
    title: 'Jazz & Greek Fusion Νύχτα',
    description: 'Σύγχρονοι ήχοι που παντρεύουν jazz με ελληνικές μελωδίες σε μια πιο ιδιαίτερη μουσική βραδιά.',
    time: '21:30',
    genre: 'Jazz Fusion / Greek',
    artist: '[Καλλιτέχνης]',
    capacity: 50,
    price: 'Ελεύθερη είσοδος',
    reservationRequired: true,
    tags: ['Jazz', 'Fusion', 'Special'],
  },
]

export const allEvents = [...upcomingEvents]

export default { recurringEvents, upcomingEvents }
